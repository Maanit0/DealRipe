 ...
 *   npx tsx scripts/test-draft.ts --account cyber    # a given account
 *   npx tsx scripts/test-draft.ts --all              # one per rep, all six
 *   npx tsx scripts/test-draft.ts --account x --full # print the whole body
 *
 * It prints the lint findings alongside the draft. A draft that ships with
 * register problems is visible here rather than only in a cron log.
 */
import { config } from "dotenv"; config({ path: ".env.local" });
import { createClient } from "@supabase/supabase-js";
import { firstNameFor, generateFollowUpDraft } from "../lib/followup-draft";
import { describeDraftFindings, draftErrors, lintDraft } from "../lib/draft-lint";
import { domainOf } from "../lib/graph-mail";

const sb = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.SUPABASE_SERVICE_ROLE_KEY!);
const arg = (n: string) => { const i = process.argv.indexOf(n); return i >= 0 ? process.argv[i + 1] : undefined; };

async function one(rep?: string, account?: string): Promise<boolean> {
  let q = sb.from("calls")
  ...
  if (account) q = q.ilike("deals.account", `%${account}%`);
  ...
  const emails = customerSide.map((p) => String(p.email ?? "").toLowerCase());
  // The greeting names, computed the way autoDraftFollowUpForCall computes
  // them. Passing them is what exercises rule 9c: without recipients the lint
  // cannot tell a person who is READING the email from one who is not, so a
  // smoke test that omitted them was silently skipping the newest rule.
  const recipients = customerSide
    .map((p, i) => firstNameFor(p.name, emails[i], transcript || null))
    .filter(Boolean)
    .join(", ");
  ...
  try {
    ...
    // Lint the SHIPPED body. Anything here survived two attempts, so it is
    // what the rep will actually open.
    const left = draftErrors(lintDraft({
      body: draft.body, subject: draft.subject,
      recipientNames: recipients.split(",").map((n) => n.trim()).filter(Boolean),
    }));
    console.log(`  ok   ${label}${left.length ? `   LINT: ${describeDraftFindings(left)}` : "   lint clean"}`);
...
(async () => {
  ...
  const reps = process.argv.includes("--all")
    ...
    : [arg("--rep")];
  let ok = true;
  for (const r of reps) ok = (await one(r, account)) && ok;
