...
/** The mined moves as a module lib/mined-plays.generated.ts, overwritten whole. */
async function writePlaybook(moves: Move[]): Promise<void> {
  const fs = await import("node:fs/promises");
  // Only seller moves, and only where the quote survived verification, which is
  // already true of everything in `moves`. Sorted so the diff is stable across
  // runs and a reviewer sees what actually changed rather than a reshuffle.
  const rows = [...moves].sort(
    (a, b) =>
  ...
  const body = rows
    .map((m) =>
      ...
      JSON.stringify({
        ...
        stage: m.stage,
        callDate: m.callDate,
        precededAdvance: m.advanced.value === "yes",
        nextMeetingInAWeek: m.nextMeeting.value === "yes",
      }),
    )
    .join(",\n");
  const out = `/**
 ...
 *
 * The moves Magaya's reps actually made, verbatim, each quote checked back
 * against its own transcript with quoteAppearsIn before it was kept. The
 * hand-curated counterpart is lib/magaya-plays.ts, and both feed the same seam.
 *
 * REVIEW THE DIFF. These are real sentences from real calls and they reach a
 * briefing a rep reads aloud to a customer. A quote carrying another
 * customer's name, a price or anything a rep should not repeat does not belong
 * here, and this file is committed precisely so a person sees it first.
 *
 * precededAdvance is CONTEXT, NEVER CAUSE. The move happened before the stage
 * moved; nothing here holds anything constant and one call is one of several.
 */
...
`;
  await fs.writeFile("lib/mined-plays.generated.ts", out, "utf8");
  console.log(`\n  WROTE lib/mined-plays.generated.ts with ${rows.length} move(s). Review the diff before shipping it.`);
}
