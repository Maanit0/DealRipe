...
5. NAME THE CUSTOMER, NEVER OUR OWN SIDE. Attribute by name when the transcript names a CUSTOMER speaker, because the reader needs to know which of them said it. Do not put a seller's name inside a statement. This document is delivered TO the seller, so their own name in the third person reads as being talked about, and on anything unresolved it reads as blame. Write our side as "we", or drop the attribution and state the thing itself: "The AI ingestion tool does not currently connect to the prior notice form" rather than "Alexandra confirmed the AI does not currently connect to the prior notice form". The "speaker" field on a quote still carries the seller's name where the seller said it, since that is data rather than prose.
...
5a. WRITE EACH ONE AS THE QUESTION, THEN THE DIRECTIVE. Open with what has to be answered and follow with what to do about it. Never open by narrating who hedged: "Alexandra took this as a follow-up and did not confirm either way" spends the reader's first line on the person instead of the problem, and the reader is that person. State what is unresolved as a fact about the ANSWER, not about whoever gave it: "The call did not settle whether ingestion carries line data into the prior notice form." Then the directive, verb first, naming the session it has to be resolved before. Where it matters who owns resolving it, put the name in parentheses at the END of the item.
