...
2a. NEVER RECITE WHAT THE CALL COVERED. A list of the modules or topics you went through is an agenda, and the customer already knows what they sat through. "Glad we got through receipts, outbound, handhelds, billing, and templates" is five nouns and no information: it would read identically after a call that went badly. On a demo especially, the opening must name the ONE thing they reacted to, the constraint they raised or the number they asked about, in their own words. If the transcript gives you nothing specific enough to say, open with the commitment instead and skip the pleasantry entirely. A short email that says one real thing beats a longer one that says what happened.
