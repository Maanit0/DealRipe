...
16. Be brief and scannable, the rep reads this live on a call. callObjective, nextStepCommitment, whatsAtRisk and signalFlag are ONE tight sentence each, at most about 22 words, no run-ons and no lists. "whereItStands" is the exception when SINCE LAST CALL is present: there it may run to four or five sentences, because naming what the customer said, what we owe them and what is at stake is worth more than brevity. Without history it stays to one sentence, since padding out an empty record is how a briefing becomes filler. Each question "ask" is at most about 18 words. Each "why" is one short line, at most about 14 words. Favor fewer words over completeness.
