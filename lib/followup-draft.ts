...
7. Match the sample voice for GREETING, SIGN-OFF, FORMALITY and word choice. Do NOT take length or structure from the samples, and this is a correction: the old rule said "if the samples are terse, be terse", which quietly overrode rule 3. The samples are a rep's ordinary mail, mostly scheduling notes and quick replies written from a phone, so matching their length produces a scheduling note after a ninety minute discovery. Eduardo's own samples skew short and his post-call emails run three times longer, because the call earned it. Rule 3 governs length; rule 7 governs how the person sounds.
...
11a. NEVER PROMISE ANOTHER EMAIL. Do not write "in a separate email", "in a follow-up email", "I'll send that shortly", or any other future correspondence. You are writing the email the rep sends; a second one does not exist unless the rep decides to write it, and inventing it creates an obligation they did not make and a promise to the customer they may not keep. The Orvia draft said "the questionnaire is on its way in a separate email", which is DealRipe committing Ariel to homework nobody agreed to. A COMMITMENT THE REP ACTUALLY MADE IS NOT AN INVENTION. Eduardo writes "I'll send over a mutual NDA shortly, once signed we can share more detail on pricing", and that is honest because he said it on the call and he is the one who sends it. What rule 11a forbids is a promise nobody made and any claim about a SEPARATE EMAIL carrying it. Restate the rep's own commitment in the rep's own words, with whatever timeframe they gave. If the rep committed on the call to send something, there are exactly two honest ways to write it: name it in attachmentsToAdd and, ONLY IF IT IS ONE OF THE COLLATERAL BUNDLE'S OWN FILES, refer to it in the present tense because DealRipe attaches those before the rep opens the draft; or, for anything else, state the commitment with the date the rep gave, in the rep's control, as "I'll get the questionnaire to you by Monday morning". Never a promise about a message.
   ...
   Do NOT write "is on its way", "are on their way" or "I'll send it over" for anything, including something included in THIS email. The ABC Cargo draft opened "The NDA and the presentation deck are both on their way to you today", which tells the customer to expect a second message that does not exist. Those describe a separate, later email and leave the customer waiting for one that never arrives.
...
function buildUserMessage(
  ...
  return [
    ...
    hasThread
      ? `THIS IS A REPLY on an existing thread. Do not re-introduce yourself and do not repeat what is already written in the thread below.
A REPLY IS STILL A POST-CALL EMAIL. Replying on an existing thread does NOT make this a continuation of that thread's topic, and it does not remove the recap. The thread here is usually the scheduling exchange that set the meeting up, so nothing in it covers what was said on the call. Eduardo Bencomo replied on exactly such a thread on 2026-09-04 and wrote a full recap of the call underneath it, because the customer has the meeting invite in writing and not the conversation.`
      : "",
    threadBody
